<template>
  <div class="container mx-auto px-4 sm:px-6 py-8" :class="{ 'mb-20': isMobile }">
    ...
    >
      ...
      >
        ...
        <!-- Tags section -->
        <div class="flex items-center mb-2 space-x-2" v-for="(tag, tagIndex) in image.tags" :key="tagIndex">
          <input
            v-model="tag.value"
          ...
          />
          <button @click="removeTag(index, tagIndex)" class="text-red-700 hover:text-red-800 text-xs">
            <i class="fas fa-times"></i>
          </button>
        </div>
        <div class="flex items-center">
          <input v-model="newTag" placeholder="Add Tag" class="flex-grow mr-2 bg-transparent focus:outline-none placeholder-gray-500 text-xs border-b-2 border-gray-300 focus:border-blue-500">
          <button @click="addTag(index)" class="text-blue-700 hover:text-blue-800 text-lg">
            +
          </button>
        </div>
      </div>
    ...
    <div v-if="showModal" class="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div class="bg-white p-6 rounded shadow-lg w-11/12 md:w-1/2 z-10">
        ...
        <div class="mt-4 flex justify-end space-x-4">
          <button @click="confirmRemove" class="bg-red-700 hover:bg-red-800 text-white py-2 px-4 rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-opacity-50">
            Delete
          </button>
          <button @click="cancelRemove" class="bg-gray-600 hover:bg-gray-700 text-white py-2 px-4 rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50">
            Cancel
          </button>
        </div>
      </div>
