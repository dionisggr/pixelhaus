<template>
  <div class="p-4 md:p-8 max-w-6xl mx-auto text-gray-700 z-10">
    ...
    >
      ...
      <div class="my-6 mx-4 flex flex-col md:flex-row justify-between">
        ...
        <button
          v-else
          @click="removeFromCart(selectedArt)"
          class="inline-flex items-center justify-center px-4 py-1 md:px-5 border border-transparent text-xs md:text-sm font-medium rounded-full shadow-sm text-white bg-gradient-to-r from-red-600 to-red-400 hover:from-red-700 hover:to-red-500 focus:outline-none focus:ring-offset-2 focus:ring-red-500 active:bg-red-700 transition ease-in-out duration-150 w-full md:w-auto mt-3 md:mt-0 md:ml-3"
        >
  ...
  <div
    v-if="notification"
    class="fixed top-12 left-1/2 transform -translate-x-1/2 bg-green-600 text-white p-4 py-2 rounded-lg shadow-lg z-30"
  >
    {{ notification }}
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    isInCart() {
      return !!this.cart.find((item) => item.id === this.selectedArt.id);
    },
  },
  methods: {
    toggleExpand() {
    ...
    removeFromCart(art) {
      if (confirm('Are you sure you want to remove this item?')) {
        this.$emit('remove-from-cart', art);
